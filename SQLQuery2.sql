...
-----store procrdure for password and username-------------------

Create Proc Sp_MST_User_SelectByUserNamePassword
	@UserName NVARCHAR(100),
    @Password NVARCHAR(100)
AS
BEGIN
    SELECT [dbo].MST_User.UserName,[dbo].MST_User.Password
    FROM [dbo].MST_User
    WHERE UserName = @UserName or Email=@UserName or Mobile=@UserName and
			Password=@Password;
END;
...
--6
EXEC Sp_MST_User_SelectByUserNamePassword 
@UserName = 'afsana', 
@Password = 'Password456'
...
CREATE PROCEDURE Sp_MST_Quiz_SelectfromDropdown
...
-- Stored Procedure: Select Question Level by SelectfromDropdown
CREATE PROCEDURE Sp_MST_QuestionLevel_SelectfromDropdown
